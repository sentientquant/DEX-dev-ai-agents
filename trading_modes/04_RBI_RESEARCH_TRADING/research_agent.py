...
def check_and_sleep_if_needed():
    """Check if we have 100+ ideas and sleep for 4 hours if so"""
    idea_count = count_ideas_in_file()

    if idea_count >= 100:
        cprint("\n" + "=" * min(60, TERM_WIDTH), "white", "on_blue")
        cprint(" 🎯 MILESTONE REACHED: 100 IDEAS! 🎯 ", "white", "on_green")
        cprint("=" * min(60, TERM_WIDTH), "white", "on_blue")
        cprint(f"\n📊 Total ideas generated: {idea_count}", "yellow")
        cprint("😴 Taking a 4-hour break to let the RBI agent process these ideas...", "cyan")
        ...
        cprint("🔄 Resuming idea generation...\n", "cyan")
