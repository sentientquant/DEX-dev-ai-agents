...
# Paper trading and risk monitoring imports
...
class TradingAgent:
    def __init__(self):
        ...
        if PAPER_TRADING_MODE:
            cprint("\n📄 Initializing Paper Trading System...", "yellow", attrs=['bold'])
            ...
            if USE_RISK_MONITORING:
                cprint("🛡️ Initializing Risk Monitoring...", "yellow")
                ...
                cprint("✅ Risk monitoring enabled (auto-close on HIGH risk)", "green")

            cprint("✅ Paper Trading Mode ACTIVE", "green", attrs=['bold'])
            cprint(f"   Starting balance: ${PAPER_TRADING_BALANCE:,.2f}", "cyan")
            cprint(f"   Max positions: {PAPER_TRADING_MAX_POSITIONS}", "cyan")
        else:
            cprint("\n⚠️  LIVE TRADING MODE - Real money at risk!", "red", attrs=['bold'])
            ...
            if current_position > 0:
                ...
                if action == "SELL":
                    ...
                    if USE_PORTFOLIO_ALLOCATION:
                        ...
                        try:
                            if PAPER_TRADING_MODE and self.paper_system:
                                # PAPER TRADING MODE - Use integrated paper trading system
                                cprint(f"📄 Executing via Paper Trading System", "cyan")
                                account_balance = self.paper_system.paper_trader.balance
                                position_size = account_balance * (MAX_POSITION_PERCENTAGE / 100)
                                ...
                                if success:
                                    cprint(f"✅ Paper trade executed: {msg}", "white", "on_green")
                                else:
                                    cprint(f"❌ Paper trade failed: {msg}", "white", "on_red")

                            else:
                                # LIVE TRADING MODE - Use direct exchange calls
                                cprint(f"⚠️  LIVE TRADING - Executing on real exchange", "red", attrs=['bold'])
                                account_balance = get_account_balance()
                                position_size = calculate_position_size(account_balance)
