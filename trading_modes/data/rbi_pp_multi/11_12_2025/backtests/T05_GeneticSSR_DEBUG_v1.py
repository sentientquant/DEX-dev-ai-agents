...
class GeneticSSR(Strategy):
    ...
    def next(self):
        ...
        macd_bullish_cross = (self.macd[-2] < self.macdsignal[-2] and self.macd[-1] > self.macdsignal[-1])
        macd_bearish_cross = (self.macd[-2] > self.macdsignal[-2] and self.macd[-1] < self.macdsignal[-1])
        long_condition = (
            self.rsi[-1] < 30 and  # Oversold
            macd_bullish_cross and  # Bullish MACD cross
        ...
        short_condition = (
            ...
            macd_bearish_cross and  # Bearish MACD cross
...
if __name__ == "__main__":
    ...
    # CRITICAL: Clean column names as per requirements
    data.columns = data.columns.str.strip().str.lower()
    data = data.drop(columns=[col for col in data.columns if 'unnamed' in col.lower()])
    
    # Ensure datetime column exists and process
    ...
    available_ohlcv = [col for col in ohlcv_lower if col in data.columns]
    if available_ohlcv:
        data = data[available_ohlcv]
        rename_dict = {col: col.capitalize() for col in available_ohlcv}
        data.rename(columns=rename_dict, inplace=True)
    else:
        print("⚠️ Warning: OHLCV columns not found. Using default columns.")
