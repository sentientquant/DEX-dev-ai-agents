...
SWARM_MODELS = {
    # 🌙 Moon Dev's Active Swarm Models - 5 Model Configuration (User Specified)
    # PERMANENT FIX: Replaced OpenRouter models with working providers (OpenRouter out of credits)
    "groq": (True, "groq", "qwen/qwen3-32b"),  # Qwen 3 32B via native Groq API - Ultra fast!
    "groq_mixtral": (True, "groq", "mixtral-8x7b-32768"),  # Mixtral 8x7B via Groq - Fast reasoning!
    "xai": (True, "xai", "grok-4-fast-reasoning"),  # Grok via native xAI API
    "groq_gemma": (True, "groq", "gemma2-9b-it"),  # Gemma 2 9B via Groq - Google's model!
    "groq_llama": (True, "groq", "llama-3.3-70b-versatile"),  # Llama 3.3 70B via Groq - 128k context!
...
CONSENSUS_REVIEWER_MODEL = ("groq", "llama-3.3-70b-versatile")  # Using Llama 3.3 70B via Groq (fast & reliable)
