...
class SqueezeBreakout(Strategy):
    ...
    def next(self):
        ...
        is_lowest_width = (not np.isnan(prev_bb_width) and not np.isnan(min_width) and
                           np.isclose(prev_bb_width, min_width, atol=1e-10))

        # Check for squeeze end and low width
        squeeze_ended = prev_squeeze and not current_squeeze and is_lowest_width
        if squeeze_ended:
            print(f"🌙 Moon Dev: Squeeze ended detected at {current_price:.2f}, vol_osc {current_vol_osc:.2f}, prev_bb_width {prev_bb_width:.4f}, min_width {min_width:.4f} 🔍✨")
