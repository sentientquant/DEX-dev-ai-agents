    ...
    def next(self):
        ...
        if not self.position and in_squeeze:
            if (current_price > self.bb_upper[-1] and
                ...
                print(f"🌙 Moon Dev: Calculated long params - Price: {current_price:.5f}, SL: {sl_price:.5f}, TP: {tp_price:.5f}, Size: {size:.4f} ✨")
                
                if size > 0 and not np.isnan(size) and not np.isnan(sl_price) and not np.isnan(tp_price) and sl_price < current_price and tp_price > current_price:
                    self.buy(size=size, sl=sl_price, tp=tp_price)
                    self.entry_bar = self.bar_count
                    print(f"🌙 Moon Dev: Long Entry Executed 🚀 Price: {current_price:.2f}, Size: {size:.4f}, SL: {sl_price:.2f}, TP: {tp_price:.2f} ✨")
                else:
                    print(f"🌙 Moon Dev: Skipping invalid long trade - Size: {size}, SL: {sl_price}, TP: {tp_price}, Price: {current_price} ✨")
        ...
        if not self.position and in_squeeze:
            if (current_price < self.bb_lower[-1] and
                ...
                print(f"🌙 Moon Dev: Calculated short params - Price: {current_price:.5f}, SL: {sl_price:.5f}, TP: {tp_price:.5f}, Size: {size:.4f} ✨")
                
                if size > 0 and not np.isnan(size) and not np.isnan(sl_price) and not np.isnan(tp_price) and sl_price > current_price and tp_price < current_price:
                    self.sell(size=size, sl=sl_price, tp=tp_price)
                    self.entry_bar = self.bar_count
                    print(f"🌙 Moon Dev: Short Entry Executed 📉 Price: {current_price:.2f}, Size: {size:.4f}, SL: {sl_price:.2f}, TP: {tp_price:.2f} ✨")
                else:
                    print(f"🌙 Moon Dev: Skipping invalid short trade - Size: {size}, SL: {sl_price}, TP: {tp_price}, Price: {current_price} ✨")
