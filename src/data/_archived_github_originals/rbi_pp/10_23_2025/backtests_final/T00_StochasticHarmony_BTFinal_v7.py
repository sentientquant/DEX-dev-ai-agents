...
class StochasticHarmony(Strategy):
    ...
    def init(self):
        ...
        self.k = self.I(lambda close: talib.STOCHRSI(close, timeperiod=14, fastk_period=14, fastd_period=3)[0], self.data.Close)
        self.d = self.I(lambda close: talib.STOCHRSI(close, timeperiod=14, fastk_period=14, fastd_period=3)[1], self.data.Close)
                        ...
                        if (abs(retr_ab - 0.618) < self.tolerance and
                            ...
                            if bull_cross and oversold and not self.position:
                                ...
                                if risk > 0:
                                    ...
                                    print(f"🌙 Moon Dev: Bullish Gartley Entry! LONG at {entry:.2f}, SL: {sl:.2f}, TP: {tp:.2f}, Size: {size:.4f} 🚀✨")
                        ...
                        if (abs(retr_ab - 0.618) < self.tolerance and
                            ...
                            if bear_cross and overbought and not self.position:
                                ...
                                if risk > 0:
                                    ...
                                    print(f"🌙 Moon Dev: Bearish Gartley Entry! SHORT at {entry:.2f}, SL: {sl:.2f}, TP: {tp:.2f}, Size: {size:.4f} 🚀✨")

        # Debug print for Stochastic RSI occasionally
        if n % 100 == 0:
            print(f"🌙 Debug: Bar {n}, K: {self.k[-1]:.2f}, D: {self.d[-1]:.2f}, ATR: {self.atr[-1]:.2f}, Pivots: {len(self.pivots)} ✨")
