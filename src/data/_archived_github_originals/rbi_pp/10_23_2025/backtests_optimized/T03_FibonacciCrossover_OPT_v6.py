...
class FibonacciCrossover(Strategy):
    ...
    def init(self):
        ...
        print("🌙 Moon Dev: Initialized Further Optimized FibonacciCrossover Strategy with Trailing Stops, Looser Filters, and Enhanced Risk for 50% Target ✨")
    ...
    def next(self):
        ...
        if (not self.position and
            ...
            current_rsi > 50 and  # 🌙 Moon Dev: Loosened RSI from >55 to >50 for broader bullish momentum confirmation, allowing more entries
            current_close > self.sma200[-1] and
            current_volume > 1.5 * self.volume_ma[-1] and  # 🌙 Moon Dev: Reduced volume filter from 2x to 1.5x MA to include more conviction entries without over-restricting
            current_adx > self.adx_threshold and  # 🌙 Moon Dev: Retained ADX filter with lowered threshold for more trending market entries
            harmonic_long):
            ...
            position_size = risk_amount / sl_distance  # 🌙 Moon Dev: Changed to float for precise position sizing, improving capital efficiency
            ...
            print(f"🌙 Moon Dev: LONG Entry at {current_close} | SL: {sl_price} | TP: {tp_price} | Size: {position_size:.4f} | ADX: {current_adx:.2f} 🚀 Enhanced Entry with Trailing Potential Confirmed ✨")
        ...
        # Short Entry - Retained core conditions with looser RSI (<50), volume (1.5x), and ADX (>20) for more high-quality trades
              ...
              current_rsi < 50 and  # 🌙 Moon Dev: Loosened RSI from <45 to <50 for broader bearish momentum confirmation, allowing more entries
              current_close < self.sma200[-1] and
              current_volume > 1.5 * self.volume_ma[-1] and  # 🌙 Moon Dev: Reduced volume filter from 2x to 1.5x MA to include more conviction entries without over-restricting
              current_adx > self.adx_threshold and  # 🌙 Moon Dev: Retained ADX filter with lowered threshold for more trending market entries
              harmonic_short):
            ...
            self.tp = tp_price
            self.entry_price = current_close  # 🌙 Moon Dev: Set entry price for trailing stop calculations
            print(f"🌙 Moon Dev: SHORT Entry at {current_close} | SL: {sl_price} | TP: {tp_price} | Size: {position_size:.4f} | ADX: {current_adx:.2f} 📉 Enhanced Entry with Trailing Potential Confirmed ✨")
