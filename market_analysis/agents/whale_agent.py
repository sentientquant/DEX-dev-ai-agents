...
class WhaleAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        # Binance API endpoints (FREE - no API key required)
    ...
    def load_history(self):
        ...
        try:
            ...
            self.oi_history = pd.DataFrame(columns=['timestamp', 'btc_oi', 'eth_oi', 'sol_oi', 'total_oi',
                                                  'btc_change_pct', 'eth_change_pct', 'sol_change_pct', 'total_change_pct'])
            print("⚠️ Created empty history due to error")
            
    def _save_oi_data(self, timestamp, btc_oi, eth_oi, sol_oi, total_oi):
        ...
        try:
            ...
            print(f"  SOL OI: ${sol_oi:,.2f}")
            print(f"  Total OI: ${total_oi:,.2f}")

            # Calculate percentage changes if we have previous data
            btc_change_pct = eth_change_pct = sol_change_pct = total_change_pct = 0.0
            ...
            if not self.oi_history.empty:
                ...
                sol_change_pct = ((sol_oi - prev_data.get('sol_oi', sol_oi)) / prev_data.get('sol_oi', sol_oi)) * 100 if 'sol_oi' in prev_data else 0.0
                ...
                print(f"SOL Change: {sol_change_pct:.4f}%")
    ...
    def _get_current_oi(self):
        """Get current open interest data from Binance Free API"""
        try:
            print("\n🔍 Fetching fresh OI data from Binance Free API...")

            # Get BTC Open Interest and Price
            btc_oi_response = requests.get(self.binance_oi_url,
                                          params={"symbol": "BTCUSDT"},
                                          timeout=10)
            btc_price_response = requests.get(self.binance_price_url,
                                             params={"symbol": "BTCUSDT"},
                                             timeout=10)

            # Get ETH Open Interest and Price
            eth_oi_response = requests.get(self.binance_oi_url,
                                          params={"symbol": "ETHUSDT"},
                                          timeout=10)
            eth_price_response = requests.get(self.binance_price_url,
                                             params={"symbol": "ETHUSDT"},
                                             timeout=10)

            # Get SOL Open Interest and Price
            sol_oi_response = requests.get(self.binance_oi_url,
                                          params={"symbol": "SOLUSDT"},
                                          timeout=10)
            sol_price_response = requests.get(self.binance_price_url,
                                             params={"symbol": "SOLUSDT"},
                                             timeout=10)

            # Check if all requests succeeded
            if (btc_oi_response.status_code == 200 and
                btc_price_response.status_code == 200 and
                eth_oi_response.status_code == 200 and
                eth_price_response.status_code == 200 and
                sol_oi_response.status_code == 200 and
                sol_price_response.status_code == 200):
                ...
                sol_oi_data = sol_oi_response.json()
                sol_price_data = sol_price_response.json()

                # Calculate OI in USD (openInterest * price)
                btc_oi = float(btc_oi_data['openInterest']) * float(btc_price_data['price'])
                eth_oi = float(eth_oi_data['openInterest']) * float(eth_price_data['price'])
                sol_oi = float(sol_oi_data['openInterest']) * float(sol_price_data['price'])
                total_oi = btc_oi + eth_oi + sol_oi

                # Use current timestamp
                current_time = datetime.now()

                cprint(f"✅ Binance API Success!", "green")
                print(f"  BTC OI: ${btc_oi:,.2f}")
                print(f"  ETH OI: ${eth_oi:,.2f}")
                print(f"  SOL OI: ${sol_oi:,.2f}")
                print(f"  Total OI: ${total_oi:,.2f}")
                ...
                # Save the data point
                self._save_oi_data(current_time, btc_oi, eth_oi, sol_oi, total_oi)

                return self.oi_history
            else:
                print(f"❌ Binance API Error:")
                print(f"  BTC OI: {btc_oi_response.status_code}")
                print(f"  BTC Price: {btc_price_response.status_code}")
                print(f"  ETH OI: {eth_oi_response.status_code}")
                print(f"  ETH Price: {eth_price_response.status_code}")
                return None

        except requests.exceptions.Timeout:
            print(f"❌ Binance API Timeout (>10 seconds)")
            return None
        except requests.exceptions.RequestException as e:
            print(f"❌ Binance API Request Error: {str(e)}")
            return None
        except Exception as e:
