...
class TradingDatabase:
        ...
        try:
            self.cursor.execute("""
                INSERT INTO trades (
                    ...
                    confidence, metadata, oco_order_list_id
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                ...
                swing_bars_ago, swing_strength, atr_pct, atr_multiplier,
                confidence, json.dumps(metadata) if metadata else None, oco_order_list_id
            ))
            self.conn.commit()
    ...
    def get_pnl_history(self, mode: str = None, days: int = 30) -> List[Dict]:
        """
        Get PnL history for dynamic risk calculations

        Returns:
            List of dicts with timestamp and pnl_usd for closed trades
        """
        query = """
            SELECT
                timestamp,
