...
if os.name == 'nt':
    import io
    # Fix encoding only if stdout has buffer attribute (not already wrapped)
...
class BinanceTruthAPI:
    ...
    def get_usdt_balance() -> Optional[float]:
        """
        Get REAL USDT balance from Binance account using API keys

        Returns:
            float: USDT balance if API keys configured, None otherwise
        """
        try:
            import os
            import hmac
            ...
            # Binance requires signed requests for account data
            ...
            response = requests.get(
                f"{BinanceTruthAPI.BASE_URL}/account",
                params=params,
                headers=headers,
                timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                print(f"[WARN] Binance API error: {response.status_code} - {response.text}")
                return None
    ...
    @staticmethod
    def get_ohlcv_data(symbol: str, timeframe: str = '15m', days_back: int = 3) -> Optional[pd.DataFrame]:
        """
        Get REAL OHLCV (candlestick) data from Binance
        ...
        try:
            ...
            response = requests.get(
                f"{BinanceTruthAPI.BASE_URL}/klines",
                params={
                    'symbol': symbol,
                    'interval': timeframe,
            ...
            if response.status_code == 200:
                ...
                # Keep only relevant columns and convert types
    ...
    @staticmethod
    def token_overview(symbol: str) -> Dict:
        """
        Get token overview data from Binance

        Returns:
            Dict with volume_24h_usd, market_cap_usd, spread_bps, etc.
        """
        try:
            if not symbol.endswith('USDT'):
                symbol = f"{symbol}USDT"
            ...
            if response.status_code == 200:
                ...
                orderbook = BinanceTruthAPI.get_orderbook(symbol, limit=5)
                spread_bps = 0
                if orderbook:
                ...
                return {
                    ...
                    'market_cap_usd': 100_000_000_000,  # Placeholder - Binance API doesn't provide this
                }
